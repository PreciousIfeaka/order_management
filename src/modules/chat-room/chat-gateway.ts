...
export class ChatGateWay implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleJoinChatRoom(
    ...
    const { data } = await this.chatRoomService.findChatRoom(payload.chatRoomId, userId);
    if (!data) {
  ...
  async handleMessages(
    ...
    try {
      ...
      const { data } = await this.chatRoomService.sendMessage(userId, payload.chatRoomId, payload.content);
      delete data.id;
