...
export class ChatRoomController {
  ...
  @ApiResponse({ status: 201, description: "Message sent successfully", schema: {
    example: {
  ...
  @ApiResponse({ status: 400, description: "Chat room is closed or invalid input" })
  @ApiResponse({ status: 404, description: "Chat room not found" })
  @Post("/:roomId/message")
  async sendMessage(@Body() body: SendMessageDto, @Param("roomId") roomId: string, @Req() request) {
  ...
  @ApiResponse({ status: 200, description: "Chat room retrieved successfully", schema: {
    example: {
  ...
  @ApiResponse({ status: 403, description: "Unauthorized user for this chat room" })
  @ApiResponse({ status: 404, description: "Chat room not found" })
  @Get("/:roomId")
  ...
  @ApiResponse({ status: 200, description: "Chat history retrieved successfully", schema: {
    example: {
      message: "Successfully retrieved chat history",
      chats: [
        { id: "msg001", senderId: "user123", content: "Hi there!", timestamp: "2025-01-03T12:34:56.789Z" },
        { id: "msg002", senderId: "user456", content: "Hello!", timestamp: "2025-01-03T12:35:56.789Z" }
      ],
  ...
  @ApiResponse({ status: 200, description: "Chat room closed successfully", schema: {
    example: {
  ...
  async closeChatRoom(@Body() body: UpdateChatRoomDto, @Param("roomId") roomId: string) {
